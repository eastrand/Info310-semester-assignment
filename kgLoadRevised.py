...
try:
    ...
    # Extract lists of names/types for the prompt
    entity_definitions = "\n".join([f"- {list(item.keys())[0]}: {list(item.values())[0]}" for item in schema.get('entities', [])])
    allowed_relationship_types = [list(item.keys())[0] for item in schema.get('relationships', [])]
    relationship_definitions = "\n".join([f"- {list(item.keys())[0]}: {list(item.values())[0]}" for item in schema.get('relationships', [])]) # Optional: add descriptions to prompt too
except FileNotFoundError:
    print(f"ERROR: Schema file not found at {SCHEMA_FILE_PATH}. Exiting.")
    exit()
except Exception as e:
    print(f"ERROR: Could not load or parse schema file {SCHEMA_FILE_PATH}. Error: {e}")
    exit()
...
Analyze the text and extract entities and relationships based on the following schema:

Entities to Extract:
{entity_definitions}

Relationships to Extract:
{relationship_definitions} # Optional: Include relationship descriptions

Based on the text, provide ONLY a JSON object with two keys: "entities" and "relationships".
- "entities" should be a list of objects, each with a "label" (matching one of the labels above) and a "name".
- "relationships" should be a list of objects, each with a "source" name, a "target" name, and a "type".

IMPORTANT: You MUST only use relationship types from the following exact list: {allowed_relationship_types}. Do NOT invent any other relationship types.

Example Output (using ONLY the allowed types):
{{
  "entities": [
    {{"label": "Agreement", "name": "The Dayton Accords"}},
    {{"label": "Party", "name": "Republic of Bosnia and Herzegovina"}},
    {{"label": "Topic", "name": "Ceasefire"}},
    {{"label": "Location", "name": "Dayton, Ohio"}},
    {{"label": "Date", "name": "November 21, 1995"}}
  ],
  "relationships": [
    ...
    {{"source": "The Dayton Accords", "target": "Dayton, Ohio", "type": "SIGNED_IN"}},
    {{"source": "The Dayton Accords", "target": "November 21, 1995", "type": "HAS_DATE"}}
  ]
}}
