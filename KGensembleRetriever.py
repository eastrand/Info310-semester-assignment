import os
import requests  # (left in case you still use it elsewhere)
...
OPENAI_MODEL = os.getenv("OPENAI_MODEL", "gpt-5")  # e.g. gpt-5, gpt-5-mini, gpt-5-thinking

# (Optional) default request timeout in seconds for the SDK's HTTP client
...
# ----------------------------
def _openai_call(chat_prompt_value):
    """
    Accepts a ChatPromptValue from LangChain, converts to OpenAI-style messages,
    calls Chat Completions with GPT-5, and returns assistant text.
    """
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.7,
            # max_tokens=1024,
            # Top-p, frequency/presence penalties etc. are supported here too.
            # GPT-5-only (if enabled for your account):
...
openai_runnable = RunnableLambda(_openai_call)
